...
def get_case_control_samples(path_samples):
    # format of case control sample is as follow
...
def parse_snp_matrix(path_matrix, case_samples, control_samples, method, path_out, num_process, progress_bar,
    ...
    print(green(f"[{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}]: Start performing fisher test and permutation"))
    ...
    if genes_with_error:
        ...
        print(red(f"[ERROR]: Error occurred when performing fisher/permutation for certain genes, Writing failed "
                  f"parsing genes to {path_error}"))
    ...
    print(green(f"[{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}]: Start sorting genes and adjusting p values"))
    ...
    print(green(f"[{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}]: Start calculating p permutation"))
    if path_pathway:
